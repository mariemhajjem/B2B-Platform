...
function Cart() {
  ...
  useEffect(() => {
    user && setFormData({ ...formData, ...user?.entrepriseClt, company_residence: [...user?.entrepriseClt?.company_residence?.split(",")] })
    setErr(error)
  }, [error])
  ...
  return (<>
    <Navbar />
    <Space id="capture" wrap direction="vertical" size="middle" style={{ display: "flex", justifyContent: "center", padding: "5em 10em" }}>
      ...
      <Space wrap style={{ display: "flex", alignItems: "flex-start" }}>
        ...
        <Card title="SOMMAIRE" style={{ height: "35%", width: "150%" }}>
          ...
          <Space>
            ...
            {/* <Button onClick={() => dispatch(addDevis(cart))}>Demander un devis</Button> */}
          </Space>
        </Card>
      </Space>
